...
#     sbatch --array=1-4 --export=ALL,ALG='cleanrl/ppo_continuous_action.py',ENV_ID=$env,ARGSS='--num_layers=16 --use_layer_norm --num_envs 4 --total_timesteps 5000000' launch.sh
    sbatch --array=1-10 --export=ALL,ALG='cleanrl/sac_continuous_action.py',ENV_ID=$env,ARGSS='--total_timesteps 1000000' --cpus-per-task=4 --time=2:55:00 launch.sh ## SAC Experiments Normal
done
